...
export default function DashboardHistoryPage() {
  ...
  const totalReadTime =
    readHistory?.reduce((sum, item) => sum + item.readTimeSpent, 0) || 0;
  const completedReads =
    readHistory?.filter((item) => item.scrolledPct >= 70).length || 0;
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Track your reading progress and discover articles you&apos;ve
              enjoyed
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-primary">
                {readHistory?.length
                  ? Math.round((completedReads / readHistory.length) * 100)
                  : 0}
                %
              </div>
              <div className="text-sm text-muted-foreground">
                  ...
                  <div className="flex-1 p-6">
                    ...
                    <div className="flex items-center space-x-3 mb-3">
                      {/* <Image
                            ...
                            /> */}
                      <Avatar>
                        <AvatarImage src={item.article.author.first_name} />
                        <AvatarFallback>
                          {item.article.author.first_name
                            .split(" ")
                            .map((n) => n[0])
