...
const mockApi = {
  // Admin Users
  getAdminUsers: async (filters: Record<string, unknown> = {}): Promise<{
    users: User[];
    total: number;
    page: number;
    limit: number;
    ...
    if (filters.role) {
      ...
      filteredUsers = filteredUsers.filter((user: User) => roles.includes(user.role));
    }
    
    if (filters.status) {
      const statuses = Array.isArray(filters.status) ? filters.status : [filters.status];
      filteredUsers = filteredUsers.filter((user: User) => statuses.includes(user.status));
    }
  ...
  // Admin Articles
  getAdminArticles: async (filters: Record<string, unknown> = {}): Promise<{
    articles: Article[];
    total: number;
    page: number;
    limit: number;
    ...
    if (filters.status) {
      const statuses = Array.isArray(filters.status) ? filters.status : [filters.status];
      filteredArticles = filteredArticles.filter(article => statuses.includes(article.status));
    }
    
    if (filters.approvalStatus) {
      const approvalStatuses = Array.isArray(filters.approvalStatus) ? filters.approvalStatus : [filters.approvalStatus];
      filteredArticles = filteredArticles.filter(article => 
  ...
  rejectArticle: async (articleId: string, reason: string): Promise<Article> => {
    await new Promise(resolve => setTimeout(resolve, 500));
    const article = mockArticles.find(a => a.id === articleId);
    if (!article) throw new Error('Article not found');
  ...
  getAdminStats: async () => {
    ...
    const activeUsers = mockUsers.filter((u: User) => u.status === 'active').length;
    const pendingUsers = mockUsers.filter((u: User) => u.status === 'pending_approval').length;
    const creators = mockUsers.filter((u: User) => u.role === 'creator').length;
    const admins = mockUsers.filter((u: User) => u.role === 'system_admin').length;
    
    const totalArticles = mockArticles.length;
    const publishedArticles = mockArticles.filter((a: Article) => a.status === 'published').length;
    const pendingArticles = mockArticles.filter((a: Article) => a.status === 'pending_approval').length;
    const draftArticles = mockArticles.filter((a: Article) => a.status === 'draft').length;
  ...
  // Role Requests
  getRoleRequests: async (filters: Record<string, unknown> = {}): Promise<{
    ...
    if (filters.status) {
      const statuses = Array.isArray(filters.status) ? filters.status : [filters.status];
      filteredRequests = filteredRequests.filter(request => statuses.includes(request.status));
    }
  ...
  rejectRoleRequest: async (requestId: string, reason: string): Promise<RoleRequest> => {
    await new Promise(resolve => setTimeout(resolve, 500));
    const request: RoleRequest = {
      id: requestId,
      userId: '2',
...
export const useUpdateUserStatus = () => {
  ...
  return useMutation({
    mutationFn: ({ userId, status }: { userId: string; status: User['status'] }) => 
      mockApi.updateUserStatus(userId, status),
...
export const useApproveRoleRequest = () => {
  ...
  return useMutation({
    ...
    onSuccess: (updatedRequest) => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.roleRequests.byUser(updatedRequest.userId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.admin.users({}) });
    },
  });
};
